...
//		outXmlWriter.close();
    ...
    public static void main(String[] args) throws DocumentException, IOException {
        ...
        @SuppressWarnings("unchecked")
		List<Node> nodes = manifest.selectNodes("/manifest/uses-permission[@android:name='android.permission.WRITE_EXTERNAL_STORAGE']");
        if (nodes.isEmpty() ) {
        	if(DEBUG) System.out.println("its fine");
		}
        else {
			reportError(root, node, i)
		}
		XMLWriter outXmlWriter = new XMLWriter(
				new FileWriter(file));
		outXmlWriter.write(document);
