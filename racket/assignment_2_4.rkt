...
(define(double l)
  (u-add l l))
(define(u-div a b)
  (cond[(greater? b a) (list '(0 0 0 0 ) a)]
  ...
  (let((avalue (abs value)))
    (cond[(< avalue (expt 2 (- 1 bias)))
         ...
         [else(let*((E-bias (floor(/(log avalue)(log 2))))
                ...
                (cons (if(positive? value)0 1)
                      (append(n2mu k (u2b-n-h k (inexact->exact E)))
                             (d2fix-h l 1 F))))])))
  ...
  (let*((s1 (car a))
    ...
    (define(case2 E1 E2 F1 F2)
      (let*((frac (u-mult f1 f2)))
        ...
        ;                         (take (cadr(find1 1 (append frac (make-list(- l 1 )0))))3))))
        (cons (if(=(car a) (car b)) 0 1) (num(if(=(car frac)1) (+ exp1 exp2 )
                                               (+ exp1 exp2 1))frac))))
    (case2 e1 e2 f1 f2)))

(define (num E F)
  (cond ((< E (- 1 bias)) (num (+ E 1) (shift-r  1 F)))
        ((and (= (car F) 0) (= E (- 1 bias))) (append (u2b-n-h k 0) (take(cdr F)l)))
        ((= (car F) 1) (append (u2b-n-h k (+ E bias)) (take(cdr F)l)))
        (else (num (- E 1) (drop(shift-l 1  F)1 )))))
