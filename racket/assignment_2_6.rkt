...
(define(u2b-n  r)
  (if(< r (expt 2 n))
     (u2b-n-h n r) (error "not in range")))
    ...
    (define(case2 E1 E2 F1 F2)
      (let*((frac (u-mult f1 f2)))
        (cons (if(=(car a)(car b)) 0 1) (num(if(=(car frac)1) (+ exp1 exp2 )
                                               (+ exp1 exp2 1))frac))))
    (check(case2 e1 e2 f1 f2))))
(define(check l)
  (if(equal? (take(cdr l)k)(make-list 4 1))
     (error "out of range")
     l))
...
;;;;;;;;;;;;;;;;;;;;;;;;Q11
(define(div-h a b)
  (let*((exp1 (take (cdr a)k ))
        (exp2 (take (cdr b)k ))
        (f1 (cons(if(equal? (make-list k 0) exp1) 0 1)
                 (take-right a l)))
        (f2 (cons(if(equal? (make-list k 0) exp2) 0 1)
                 (take-right b l)))
    ...
    (let*((qtn (car(u-div-h (* 2(+ l 1)) (append f1 (make-list (+ l 1)0))
      ...
      (if(<=(caar(find1 1 qtn))(+ l 1))
         ...
         (num (-(- e1 e2)1) (take-right qtn (+ l 1)))))))
(define(divide a b)
  (check(cons (if(=(car a)(car b)) 0 1) (div-h a b))))
